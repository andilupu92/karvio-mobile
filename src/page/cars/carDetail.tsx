    ...
    >
      ...
      >
        ...
        >
          ...
          <View className="flex-row gap-3 mb-6">
            <StatusCard
              icon={
                <Droplets
                  size={20}
                  color={getConsumptionColor(car.consumption, isDark).iconColor}
                  strokeWidth={1.8}
                />
              }
            ...
            <StatusCard
              icon={
                <Wallet
                  size={20}
                  color={getExpenseColor(car.amount, isDark).iconColor}
                  strokeWidth={1.8}
                />
              }
            ...
            <StatusCard
              icon={
                <ShieldCheck
                  size={20}
                  color={getHealthScoreColor(car.healthScore, isDark).iconColor}
                  strokeWidth={1.8}
                />
              }
          ...
          {documentsLoading ? (
            <ActivityIndicator size="small" color="#14b8a6" style={{ marginTop: 100 }} />
          ) : (
            documents.length > 0 && (
              <View className="mb-6">
                <Text
                  className={`${isDark ? 'text-typography-900' : 'text-typography-100'} font-inter-bold text-base mb-3`}
                >
                  Alerte documente
                </Text>

                <Box
                  className={`rounded-xl p-4 gap-2 border ${isDark ? 'bg-background-card-900 border-outline-900' : 'bg-background-card-100 border-outline-100'}`}
                >
                    ...
                    return (
                      ...
                      >
                        ...
                        <View className="flex-1">
                          ...
                          <Text
                            className="font-inter-regular text-xs mt-0.5"
                    ...
                    >
                      ...
                      <View className="flex-1">
                        <Text className="font-inter-semibold text-sm text-slate-500">
                          {laterDocs.length}{' '}
                          {laterDocs.length === 1 ? 'document valabil' : 'documente valabile'}
                        </Text>
                    ...
                    <View key={expense.id}>
                      <View className="flex-row items-center px-4 py-3">
                        ...
                        <View className="flex-1">
                          ...
                          <Text
                            className={`${isDark ? 'text-typography-800' : 'text-typography-200'} font-inter-regular text-xs mt-0.5`}
                          >
                            {formatDate(expense.date.toString())}
                          </Text>
                        </View>
                        <Text className="font-inter-bold text-sm" style={{ color: '#ef4444' }}>
                          {expense.amount} RON
                        </Text>
                      </View>
            ...
            >
              <View
                className={`w-[22px] h-[22px] rounded-md border-2 justify-center items-center ${
                  deleteExpenses ? 'border-red-500 bg-red-500' : 'border-slate-300 bg-white'
                }`}
              >
                {deleteExpenses && <Text className="text-white text-[13px] leading-4">✓</Text>}
              </View>
...
function getAlertStyle(days: number) {
  if (days <= 3) return { bg: '#fff1f0', icon: '#ef4444', text: '#ef4444', iconBg: '#fee2e2' };
  if (days <= 7) return { bg: '#fff9e4', icon: '#f59e0b', text: '#f59e0b', iconBg: '#fef3c7' };
