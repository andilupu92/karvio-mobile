...
export default function HomeHeader({
  ...
  return (
    <Box className="flex-row items-center px-4 py-3 gap-3">
      ...
      {lengthCar != 0 && (
        <View
          className={`flex-row flex-1 items-center ${isDark ? 'bg-background-card-900 border-outline-900' : 'bg-background-card-100 border-outline-100'} rounded-xl py-[11px] px-5 border`}
        >
          {/* Left — Car info */}
          <Box className="flex-row items-center gap-3 flex-1">
            ...
            <Box>
              <Text
                className={`text-[12px] font-inter-medium ${isDark ? 'text-typography-800' : 'text-typography-200'} `}
              >
                {carName}
              </Text>
              <Text
                className={`text-[14px] font-inter-bold ${isDark ? 'text-typography-900' : 'text-typography-100'}`}
              >
          ...
          {/* Divider */}
          <View
            className={`mx-[38px] h-[30px] w-[1.5px] ${isDark ? 'bg-outline-900' : 'bg-outline-100'}`}
          />
