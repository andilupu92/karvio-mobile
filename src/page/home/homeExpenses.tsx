...
export default function HomeExpenses({
  ...
  const top3 = Object.values(
    (expensesHistory?.expenseResponseList ?? []).reduce(
      (acc, item) => {
    ...
    <TouchableOpacity onPress={() => onExpensesPress()} className="flex-1" activeOpacity={0.7}>
      ...
      >
        <View className="flex-row items-center justify-between mb-3">
          ...
          />
          <View
            className={`${isDark ? 'bg-background-icon-900' : 'bg-background-icon-100'} rounded-full px-3 py-1`}
          >
            <Text
              className={`${isDark ? 'text-typography-900' : 'text-typography-100'} font-inter-medium text-xs`}
            >
              {period}
            </Text>
          </View>
              ...
              {top3.map((expense, index) => (
                <View key={expense.id} style={{ gap: 2 }}>
                  <View className="flex-row justify-between">
                    <Text
                      className={`${isDark ? 'text-typography-800' : 'text-typography-200'} text-[10px]`}
                    >
                      {expense.expenseTypeName}
                    </Text>
                    <Text
                      className={`${isDark ? 'text-typography-900' : 'text-typography-100'} text-[10px] font-inter-semibold`}
                    >
                  ...
                  {/* Track */}
                  <View
                    className={`h-[3px] ${isDark ? 'bg-background-icon-900' : 'bg-background-icon-100'} rounded`}
                  >
                    {/* Fill */}
                    <View
                      style={{
