...
export default function HomeHealth({ healthScore }: Props) {
  ...
  return (
    <Box
      className={`rounded-xl px-4 pt-4 pb-3 flex-2 border ${isDark ? 'bg-background-card-900 border-outline-900' : 'bg-background-card-100 border-outline-100'}`}
    >
      {/* Top row */}
      ...
      <Text className={`font-inter-bold text-[20px] ${healthStyle?.color}`}>
        ...
        {healthScore ? (
          <Text
            className={`text-typography-50 font-inter-medium text-[14px] ${isDark ? 'text-typography-900' : 'text-typography-100'}`}
          >
            {' '}
            %
          </Text>
