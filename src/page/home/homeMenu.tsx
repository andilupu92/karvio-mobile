...
export default function HomeMenu({ activeTab = 'Home', onTabPress, onAddPress, cars }: HomeMenuProps) {
  ...
  return (
    <View style={{ width: W, height: totalH, position: 'absolute', bottom: 0 }}>
      {/* 1. Background Wave */}
      <Svg width={W} height={totalH} style={{ position: 'absolute', bottom: 0, left: 0 }}>
      ...
      >
        ...
        >
          <Icons.Plus className="text-icons-900" size={28} strokeWidth={2} />
        </TouchableOpacity>
      </View>
        ...
        {TABS.map((tab, index) => {
          ...
          return (
            <React.Fragment key={tab.name}>
              ...
              {index === 2 && <View style={{ width: FAB_SIZE - 10 }} />}
