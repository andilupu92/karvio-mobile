        ...
        <View className="flex-row justify-center gap-28 items-start">
          {actions.map((action, index) => (
            ...
            >
              {/* White circle */}
              <View
                className={`w-12 h-12 rounded-full ${isDark ? 'bg-background-icon-900' : 'bg-background-icon-100'} items-center justify-center`}
              >
                {action.icon}
              </View>
              ...
              {/* Label */}
              <Text
                className={`text-[12px] ${isDark ? 'text-typography-900' : 'text-typography-100'} text-center font-inter-medium leading-[19px]`}
                numberOfLines={2}
              >
                {action.label}
              </Text>
