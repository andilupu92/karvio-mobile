...
export default function ExpensesDetail() {
  ...
  const visibleItems = showAll
    ? selectedMonth.expenseResponseList
    : selectedMonth.expenseResponseList.slice(0, INITIAL_VISIBLE);
          ...
          <View className="flex-1 mt-3">
            {expenses.length > 0 ? (
                  ...
                  >
                    {visibleItems.length === 0 ? (
                      <View className="py-8 items-center">
                        <Text
                          className={`${isDark ? 'text-typography-900' : 'text-typography-100'} font-inter-regular text-sm`}
                        >
                            ...
                            <View key={item.id}>
                              <View className="flex-row items-center px-5 py-4">
                                ...
                                <View className="flex-1">
                                  <Text
                                    className={`font-inter-semibold ${isDark ? 'text-typography-900' : 'text-typography-100'} text-sm`}
                                  >
                                ...
                                {/* Suma */}
                                <Text
                                  className={`font-inter-bold ${isDark ? 'text-typography-900' : 'text-typography-100'} mr-3`}
                                  style={{ fontSize: 15 }}
                                >
                                  {item.amount}{' '}
                                  <Text
                                    className={`font-inter-regular ${isDark ? 'text-typography-800' : 'text-typography-200'}`}
                                    style={{ fontSize: 11 }}
                                  >
                                    RON
                                  </Text>
              ...
              <Text className="text-center mt-20">
                Nu există date despre cheltuieli pentru mașina aceasta.
              </Text>
            )}
          </View>
