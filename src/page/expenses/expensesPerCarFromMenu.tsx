...
export default function ExpensesPerCarFromMenu({
  carResponsesList,
  onCarPress,
  totalAmountOfMonth,
}: ExpensesPerCarProps) {
    ...
    <View className="mt-6">
      ...
      >
        {carResponsesList.map((item, index) => {
          ...
          const barWidth =
            totalAmountOfMonth > 0 ? (item.totalAmount / totalAmountOfMonth) * 100 : 0;
            ...
            >
              ...
              <View className="flex-row items-center px-4 py-4">
                ...
                <View className="flex-1 mr-3">
                  <Text
                    className={`${isDark ? 'text-typography-900' : 'text-typography-100'} font-inter-semibold text-sm mb-1`}
                  >
                    {item.car.name}
                  </Text>
                ...
                <View className="items-end mr-2">
                  <Text
                    className={`${isDark ? 'text-typography-900' : 'text-typography-100'} font-inter-bold text-sm`}
                  >
                    {item.totalAmount.toLocaleString('ro-RO')}{' '}
                    <Text
                      className={`${isDark ? 'text-typography-800' : 'text-typography-200'} font-inter-regular`}
                      style={{ fontSize: 11 }}
                    >
                      RON
                    </Text>
