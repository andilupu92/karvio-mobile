...
export default function DocumentCard({ document, onPress }: Props) {
  ...
  return (
    ...
    >
      ...
      <View className="flex-row items-center">
        ...
        <View className="flex-1">
          <View className="flex-row items-center gap-2 mb-0.5">
            <Text
              className={`${isDark ? 'text-typography-900' : 'text-typography-100'} font-inter-semibold text-sm mb-0.5`}
            >
              {document.documentTypeName}
            </Text>
            {document.carName && (
              <View
                className={`rounded-md px-2 py-0.5 ml-auto mr-2 ${isDark ? 'text-typography-800' : 'text-typography-200'}`}
              >
                <Text
                  className={`${isDark ? 'text-typography-900' : 'text-typography-100'} font-inter-medium text-typography-300 text-xs`}
                >
                  {document.carName}
                </Text>
              </View>
          ...
          <Text
            className={`font-inter-medium text-xs ${getColor(document.daysRemaining)}`}
            numberOfLines={1}
          >
      ...
      {/* Progress bar */}
      <View
        className={`${isDark ? 'bg-background-icon-900' : 'bg-background-icon-100'} mt-3 h-[3px] rounded-full overflow-hidden`}
      >
        <View
